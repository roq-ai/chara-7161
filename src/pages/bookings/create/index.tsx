...
function BookingCreatePage() {
  ...
  const formik = useFormik<BookingInterface>({
    initialValues: {
      session_time: new Date(new Date().toDateString()),
      end_customer_id: (router.query.end_customer_id as string) ?? null,
      practitioner_id: (router.query.practitioner_id as string) ?? null,
    },
    ...
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        ...
        <form onSubmit={formik.handleSubmit}>
          <FormControl id="session_time" mb="4">
            <FormLabel>Session Time</FormLabel>
            <Box display="flex" maxWidth="100px" alignItems="center">
              <DatePicker
                dateFormat={'dd/MM/yyyy'}
                selected={formik.values?.session_time ? new Date(formik.values?.session_time) : null}
                onChange={(value: Date) => formik.setFieldValue('session_time', value)}
              />
              <Box zIndex={2}>
                <FiEdit3 />
              </Box>
