...
function BookingListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<BookingInterface[]>(
    ...
    () =>
      getBookings({
        relations: ['user_booking_end_customer_idTouser', 'user_booking_practitioner_idTouser'],
      }),
  );
              ...
              <Thead>
                <Tr>
                  ...
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_booking_end_customer_idTouser</Th>
                  )}
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_booking_practitioner_idTouser</Th>
                  )}
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    <Td>{record.session_time as unknown as string}</Td>
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_booking_end_customer_idTouser?.id}`}>
                          {record.user_booking_end_customer_idTouser?.email}
                        </Link>
                      </Td>
                    )}
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_booking_practitioner_idTouser?.id}`}>
                          {record.user_booking_practitioner_idTouser?.email}
                        </Link>
                      </Td>
                    )}
